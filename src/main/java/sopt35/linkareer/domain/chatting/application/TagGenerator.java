...
public class TagGenerator {
    ...
    private static final List<String> companyNames = List.of("삼성전자", "LG전자", "SK하이닉스", "네이버",
        "카카오");
    private static final List<String> jobs = List.of("개발자", "디자이너", "기획자", "마케터", "연구원");

    public TagVo pickRandomTag() {
        String companyName = companyNames.get(
            ThreadLocalRandom.current().nextInt(0, companyNames.size()));
        String job = jobs.get(ThreadLocalRandom.current().nextInt(0, jobs.size()));
